...
function FinancialProgress({ company }) {
  ...
  useEffect(() => {
    dispatch({
      type: "GET_GRAPH_DATA",
      payload: {
        fromMonth: fromDateSelected.format("MM"),
  ...
  const handleDateRangeChange = (value, mode) => {
    ...
    if (mode === "from") {
      if (newSelectedDate <= toDateSelected) {
        ...
        setErrorMsg(
          "From date is less than to date, please re-select range to see graphs"
        );
      }
    } else {
  ...
  const options = {
    ...
    scales: {
      ...
      y: {
        ...
        min:
          graphData.userMetrics &&
          graphData.industryMetrics &&
      ...
      <Box>
        ...
        <Paper elevation={10} sx={{ px: 2, pb: 3 }}>
          ...
          <Typography variant="h4" align="center">
            {graphData.metric_name && graphData.metric_name} for {company}{" "}
          </Typography>
            ...
            >
              <Box sx={{ ml: 2 }} textAlign="left">
                <DatePicker
                  ...
                  onChange={(value) => {
                    handleDateRangeChange(value, "from");
                  }}
                  views={["year", "month"]}
                  maxDate={dayjs()}
                ></DatePicker>
          ...
          <Box sx={{ display: "flex", justifyContent: "center" }}>
            {graphData ? (
              <Line key={graphData.metric_id} data={data} options={options} />
            ) : null}
          </Box>
          <Box
            sx={{
